  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap gap-1 rounded-lg border border-border-hair bg-surface/40 p-1">
        {TABS.map((t, i) => (
          <button key={t.id} onClick={() => setTab(t.id)} className={`rounded-md px-3.5 py-2 font-mono text-[13px] transition-colors ${tab === t.id ? "bg-accent text-on-accent" : "text-muted hover:text-fg"}`}>
